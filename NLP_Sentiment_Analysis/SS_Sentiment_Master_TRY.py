...
def write_data_SQL(df,server,db,tableName,schemaName='dbo',append_replace='replace'):
    ...
    df.to_sql(tableName, engine,if_exists=append_replace,schema=schemaName,index=False)
    connection.close()
...
#    df_out=calc_stanford_corenlp_sentiment(df_in)
    ...
    try:    
        ...
        write_data_SQL(df_out,server,db,'t_New_Setiment_StanfordCorenlp','CUS','append')
