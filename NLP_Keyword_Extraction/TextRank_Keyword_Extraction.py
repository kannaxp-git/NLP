...
if __name__=='__main__':
    ...
    experiences={'Accessing','Collaborating','Communicating','Finding','Meeting','Supporting'}
    for exp in experiences:
        ...
        where b.experience='"""+ exp +"""' and a.Preprocessed_responses is not null
        """
        cur.execute(qry)
        rows=cur.fetchall()
        
        # creating document by merging all rows
        doc=' '.join(row.TextForAnalysis for row in rows)
        
        #calling 'extract key phrases' function    
        keywords=extract_key_phrases(doc)
    
        #convert 'set' inta a 'list' to upload to sql server
        keywords=list(keywords)
        df_out=pd.DataFrame(keywords,columns=['keyphrase'])
    
        df_out['Experience']=exp
        print(exp+"-Completed!")
        
        write_data_SQL(df_out,server,db,'t_New_keyphrases_TextRank','CUS','append')

    #Closing connection as no longer required
    con.commit()
    con.close()
