...
#source=('GESS','SNOW')
experience=('Airline','AMEX','CAR','Hotel','Insurance','Tool','Visa')
...
for e in experience:
    print(e)
    qry="SELECT cast(left([DESCRIPTION_NORMALIZED_STOPWORDS],1000)as VARCHAR(MAX)) AS TextForAnalysis from [WORKAREA].MST.t_Sample where "+e+">=0.3"
    cur.execute(qry)
    rows=cur.fetchall()
...
#        display_topics(lda, tf_feature_names, no_top_words)
    ...
    for topic_idx, topic in enumerate(nmf.components_):
        ...
        outputstring=''.join([e,"\t","NMF","\t",str(no_topics-topic_idx),"\t",topic,"\n"])
        txtoutput.write(outputstring)
    
    for topic_idx, topic in enumerate(lda.components_):
        topic=" ".join([tf_feature_names[i] for i in topic.argsort()[:-no_top_words - 1:-1]])
        outputstring=''.join([e,"\t","LDA","\t",str(no_topics-topic_idx),"\t",topic,"\n"])
        txtoutput.write(outputstring)
