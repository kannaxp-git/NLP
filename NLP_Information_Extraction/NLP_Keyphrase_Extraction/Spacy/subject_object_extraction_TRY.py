...
def findSVOs2(tokens):
    ...
    for v in verbs:
        ...
        if len(subs) > 0:
            ...
            for obj in objs:
                objNegated = isNegated(obj)
                svos.append(("!" + v.lower_ if verbNegated or objNegated else v.lower_, obj.lower_))

        else:
            svos.append(("!" + v.lower_ if isNegated(v) else v.lower_))
...
def testSVOs():
    ...
    tok = nlp("Unable to don't connect to VPN - Error 798 - Workplace managed computer")
    print(findSVOs(tok))
    print(findSVOs2(tok))
